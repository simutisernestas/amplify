...
def inference(img_path, show=False):
    ...
    for i in range(100):
        ...
        # inference benchmark
        pred_onx = sess.run([loc_name, conf_name, mask_name, priors_name, proto_name], {
                            input_name: batch.cpu().detach().numpy()})
        print(f"Average inference duration: {(time.time()-start)/(i+1)}s")
        
        detection = layers.Detect(
            81, bkg_label=0, top_k=200, conf_thresh=0.05, nms_thresh=0.5)
    ...
    img = eval.prep_display(preds, frame.cpu(), None,
                            None, None, None, undo_transform=False)
